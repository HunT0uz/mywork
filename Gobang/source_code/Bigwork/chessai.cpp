...
int chessAi::calcOnePosGreedy(int board[15][15],int row, int col, int C_ME){
    int sum=0;
    for(int i=0;i<4;++i){//四个方向（右，上，上右，上左）
...
//遍历棋盘，选出空位，并对这些空位置计算分数，比较出最优分数位置。
QPoint chessAi::findPossPosGreedy(int C_ME){
    int bestScore=0;
    int bestRow=0,bestCol=0;
    for(int i=0;i<15;++i){
        for(int j=0;j<15;++j){
            if(chess_board[i][j]==C_NONE){//空的位置
                int score=calcOnePosGreedy(chess_board,i,j,C_ME);
                if(bestScore<score){
                    bestScore=score;
                    bestRow=i;
                    bestCol=j;
...
//初始化棋形六元组
void chessAi::init_tupletype(){
    memset(tupletype,0,sizeof (tupletype));//全部设为0
    //白连5,ai赢
    tupletype[2][2][2][2][2][2]=WIN;
    tupletype[2][2][2][2][2][0]=WIN;
    tupletype[0][2][2][2][2][2]=WIN;
    tupletype[2][2][2][2][2][1]=WIN;
    tupletype[1][2][2][2][2][2]=WIN;
    tupletype[3][2][2][2][2][2]=WIN;//边界考虑
    tupletype[2][2][2][2][2][3]=WIN;
    //黑连5,ai输
    tupletype[1][1][1][1][1][1]=LOSE;
    tupletype[1][1][1][1][1][0]=LOSE;
    tupletype[0][1][1][1][1][1]=LOSE;
    tupletype[1][1][1][1][1][2]=LOSE;
    tupletype[2][1][1][1][1][1]=LOSE;
    tupletype[3][1][1][1][1][1]=LOSE;
    tupletype[1][1][1][1][1][3]=LOSE;
    //白活4
    tupletype[0][2][2][2][2][0]=FLEX4;
    //黑活4
    tupletype[0][1][1][1][1][0]=flex4;
    //白活3
    tupletype[0][2][2][2][0][0]=FLEX3;
    tupletype[0][0][2][2][2][0]=FLEX3;
    tupletype[0][2][0][2][2][0]=FLEX3;
    tupletype[0][2][2][0][2][0]=FLEX3;
    //黑活3
    tupletype[0][1][1][1][0][0]=flex3;
    tupletype[0][0][1][1][1][0]=flex3;
    tupletype[0][1][0][1][1][0]=flex3;
    tupletype[0][1][1][0][1][0]=flex3;
    //白活2
    tupletype[0][2][2][0][0][0]=FLEX2;
    tupletype[0][2][0][2][0][0]=FLEX2;
    tupletype[0][2][0][0][2][0]=FLEX2;
    tupletype[0][0][2][2][0][0]=FLEX2;
    tupletype[0][0][2][0][2][0]=FLEX2;
    tupletype[0][0][0][2][2][0]=FLEX2;
    //黑活2
    tupletype[0][1][1][0][0][0]=flex2;
    tupletype[0][1][0][1][0][0]=flex2;
    tupletype[0][1][0][0][1][0]=flex2;
    tupletype[0][0][1][1][0][0]=flex2;
    tupletype[0][0][1][0][1][0]=flex2;
    tupletype[0][0][0][1][1][0]=flex2;
    //白活1
    tupletype[0][2][0][0][0][0]=FLEX1;
    tupletype[0][0][2][0][0][0]=FLEX1;
    tupletype[0][0][0][2][0][0]=FLEX1;
    tupletype[0][0][0][0][2][0]=FLEX1;
    //黑活1
    tupletype[0][1][0][0][0][0]=flex1;
    tupletype[0][0][1][0][0][0]=flex1;
    tupletype[0][0][0][1][0][0]=flex1;
    tupletype[0][0][0][0][1][0]=flex1;

    int p1,p2,p3,p4,p5,p6,x,y,ix,iy;//x:左5中黑个数,y:左5中白个数,ix:右5中黑个数,iy:右5中白个数
    //循环初始化6个不同的位置每个位置有四种情况（0，1，2，3）
    //棋盘内中间位置只有三种，不考虑边界
    for(p1=0;p1<4;++p1){
        for(p2=0;p2<3;++p2){
            for(p3=0;p3<3;++p3){
                            ...
                            if(p1==3||p6==3){//有边界
                                if(p1==3&&p6!=3){//左边界
                                    ...
                                    if(ix==0&&iy==4){//若右边有空位是活4也没关系，因为活4权重远大于冲4，再加上冲4权重变化可以不计
                                        if(tupletype[p1][p2][p3][p4][p5][p6]==0)
                                            tupletype[p1][p2][p3][p4][p5][p6]=BLOCK4;
                                    }
                                    //黑冲4
                                    if(ix==4&&iy==0){
                                        if(tupletype[p1][p2][p3][p4][p5][p6]==0)
                                    ...
                                    if(ix==0&&iy==3){
                                        if(tupletype[p1][p2][p3][p4][p5][p6]==0)
                                            tupletype[p1][p2][p3][p4][p5][p6]=BLOCK3;
                                    }
                                    //黑眠3
                                    if(ix==3&&iy==0){
                                        if(tupletype[p1][p2][p3][p4][p5][p6]==0)
                                    ...
                                    if(ix==0&&iy==2){
                                        if(tupletype[p1][p2][p3][p4][p5][p6]==0)
                                            tupletype[p1][p2][p3][p4][p5][p6]=BLOCK2;
                                    }
                                    //黑眠2
                                    if(ix==2&&iy==0){
                                        if(tupletype[p1][p2][p3][p4][p5][p6]==0)
...
EVALUATION chessAi::evaluate(int board[15][15]){
    ...
    for(i=1;i<=15;++i){
        for(j=0;j<12;++j){
            type=tupletype[A[i][j]][A[i][j+1]][A[i][j+2]][A[i][j+3]][A[i][j+4]][A[i][j+5]];
            stat[0][type]++;
        }
    }
    //判断竖向棋型
    for(j=1;j<=15;++j){
        for(i=0;i<12;++i){
            type=tupletype[A[i][j]][A[i+1][j]][A[i+2][j]][A[i+3][j]][A[i+4][j]][A[i+5][j]];
            stat[1][type]++;
        }
    }
    //判断左上至右下棋型
    for(i=0;i<12;++i){
        for(j=0;j<12;++j){
            type=tupletype[A[i][j]][A[i+1][j+1]][A[i+2][j+2]][A[i+3][j+3]][A[i+4][j+4]][A[i+5][j+5]];
            stat[2][type]++;
        }
    }
    //判断右上至左下棋型
...
POINTS chessAi::seekPoints(int board[15][15],int flag,int depth){
    ...
    for(int i=0;i<15;++i){//每个非空点附近8个方向延伸3个深度,若不越界则标记为可走
        for(int j=0;j<15;++j){
            if(board[i][j]!=C_NONE&&myChess[i][j]!=0){
                for(int k=-3;k<=3;++k){
                    if(i+k>=0&&i+k<15){
                        B[i+k][j]=true;
    ...
    for(int i=0;i<15;++i){
        for(int j=0;j<15;++j){
            worth[i][j]=-INT_MAX;
            if(board[i][j]==C_NONE&&B[i][j]==true&&myChess[i][j]==0){
    ...
    for(int k=0;k<couWorth;++k){
        ...
        if(zobb.probe(hashkey,depth,rescore,reflag)){
            ...
            best_points.score[k]=evaluate(board1).score;
...
int chessAi::analyse(int (*board)[15], int depth,int alpha, int beta){
    ...
    if(depth==0||EVAL.result!=R_DRAW){//抵达最深层/如果模拟落子可以分出输赢，那么直接返回结果，不需要再搜索
        ...
        POINTS P=seekPoints(rBoard,BETA,depth);
        //找对于黑子的最佳位置,需要将棋盘不同颜色反转,因为seekPoint是求白色方的最佳位置
