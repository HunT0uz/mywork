...
function submit() {
  formRef.value.validate((valid) => {
    ...
    if (!formData.value.id) {
      request.post("/productType/add", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
      ...
      //更新
      request.put("/productType/update", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
...
function batchDelete(rows) {
  ...
  ElMessageBox.confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', {
    ...
    request.delete("/productType/delBatch", {data: ids}).then(res => {
      if (!res) {
        return
      }
      ElMessage({
